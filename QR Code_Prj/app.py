...
class ValidatorClass:	# form validator class
	...
	def check_id(self,strg):
		if(len(strg)!=10):
			return False
		if(not(strg[0]=='2' or strg[2]=='a' or strg[2]=='A' or strg[3]=='j' or strg[3]=='J' or strg[4]=='1' or strg[5]=='a' or strg[5]=='A' )):
			return False
...
class QRGenerator(Window):
	...
	def __init__(self,mainWin,title,geometry):
		...
		yearDropDown = OptionMenu(formFrame,self.yearVal,*["E1","E2","E3","E4"])
...
class QRScanner(Window):
	def __init__(self,mainWin,title,geometry):
		try:
			...
			self.cap = cv2.VideoCapture(0)	# instance of camera input reader
			self.detector = cv2.QRCodeDetector()	# instance of QR code detector.
	...
	def addStudent(self):
                ...
                if validator.check_id(self.idVal.get()):
                        ...
                        Mobile=self.mobVal.get()
                        Parent=self.parMobVal.get()
                        year=self.yearVal.get()
                        Outtime=self.timeVal.get()
                        Intime=datetime.now().strftime("%H:%M:%S")
                        query = "INSERT INTO students (ID,Name,Mobile,Parent,year,Outtime,Intime) VALUES (%s, %s,%s, %s,%s, %s,%s)"
                        values = (ID,Name,Mobile,Parent,year,Outtime,Intime)
